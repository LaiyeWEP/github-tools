...
class GiteaToolsTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            # 首先获取PR信息，以获取目标分支
            pr_url = f"{gitea_api_url}/api/v1/repos/{owner}/{repo}/pulls/{mr_number}"
            ...
            if not head_sha:
                yield self.create_text_message("Error: Could not determine PR head commit")
                return
            
            # 如果提供了特定文件路径，则只获取该文件的内容
            if file_path:
                content = self._get_file_content(gitea_api_url, owner, repo, file_path, head_sha, headers)
                if content:
            ...
            # 如果没有提供文件路径，则获取MR中所有文件的内容
            else:
                # 获取MR中修改的文件列表
                files_url = f"{gitea_api_url}/api/v1/repos/{owner}/{repo}/pulls/{mr_number}/files"
                ...
                for path in file_paths:
                    content = self._get_file_content(gitea_api_url, owner, repo, path, head_sha, headers)
                    if content:
